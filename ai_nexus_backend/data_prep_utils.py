"""
Functions to help transform data for the service catalogue into a format
...
def fetch_data(fname: str) -> list:
    """
    Read data from a json file into a list. Perform some basic data
    cleaning.
...
def transform_data(project_list: list) -> list:
    ...
    Return
    List of dictionaries, one for each field to search over for each
    project.
    """
    ...
    # Iterate through the list of projects and reformat to more easily
    # allow us to search over multiple fields
    dataset = [
        y
        for project in project_list
